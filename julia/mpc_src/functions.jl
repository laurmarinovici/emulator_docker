...
mutable struct Params
    ...
    zoneflow_min::Vector{Float64}          # of length numzones
    zoneflow_max::Vector{Float64}          # of length of numzones
    ...
    function Params()
        ...
        obj.zoneflow_min = ones(obj.numzones)
        obj.zoneflow_max = ones(obj.numzones)
        for z in 1:obj.numzones
            obj.zoneflow_min[z] = parse_float(conf, section, "zone$(z)_flow_min")
            obj.zoneflow_max[z] = parse_float(conf, section, "zone$(z)_flow_max")
        end
        ...
        # fan parameters
        section = "fan_params"
        obj.fan_params = ones(4)
        for i in 1:4
            obj.fan_params[i] = parse_float(conf, section, "fan_param$(i)")
        end

        # static pressure parameters
        section = "pressure_damper_params"
        obj.pressure_min = parse_float(conf, section, "pressure_min")
        obj.pressure_max = parse_float(conf, section, "pressure_max")
        obj.damper_min = parse_float(conf, section, "damper_min")
        obj.damper_max = parse_float(conf, section, "damper_max")
...
@variable(m, ahudamper[f = 1:p.numfloors, h = 1:o.numwindows],
            lower_bound = p.damper_min, upper_bound = p.damper_max)
...
@variable(m, zoneflow[f = 1:p.numfloors, z = 1:p.numzones, h = 1:o.numwindows],
             lower_bound = p.zoneflow_min[z], upper_bound = p.zoneflow_max[z])
...
@NLexpression(m, fanenergy[f = 1:p.numfloors, t = 1:o.numstages],
                (p.fan_params[1] + p.fan_params[2] * sum_zoneflows[f, t] + p.fan_params[3] * (sum_zoneflows[f, t])^2 + p.fan_params[4] * ahupressures[f, t]))
...
if o.modelflag == 1
    ...
    @NLobjective(m, Min,
                60.0 * sum(o.weight_heat * heatingcapacity[f, t] + o.weight_cool * chillercapacity[f, t] + o.weight_fan  * fanenergy[f, t] for f = 1:p.numfloors, t = 1:o.numstages) + o.penalty * slack^2)
...
function setoverrides!(df::DataFrames.DataFrame;
    ...
    if control == "MPC"
        for f = 1:p.numfloors
                ...
                df[Symbol("floor$(f)_aHU_con_oveMinOAFra_u")] = JuMP.value(ahudamper[f, currStage])

                # ahu supply temperatures
                df[Symbol("floor$(f)_aHU_con_oveTSetSupAir_activate")] = 1
                if unit == "Kelvin"
                    df[Symbol("floor$(f)_aHU_con_oveTSetSupAir_u")] = JuMP.value(ahusupplytemp[f, currStage]) + 273.15
                else
                    df[Symbol("floor$(f)_aHU_con_oveTSetSupAir_u")] = JuMP.value(ahusupplytemp[f, currStage])
                end
                ...
                for z = 1:p.numzones
                    ...
                    df[Symbol("floor$(f)_zon$(z)_oveAirFloRat_u")] = JuMP.value(zoneflow[f, z, currStage])/p.zoneflow_max[z]

                    df[Symbol("floor$(f)_zon$(z)_oveHeaOut_activate")] = 1
                    df[Symbol("floor$(f)_zon$(z)_oveHeaOut_u")] = (JuMP.value(zonedischargetemp[f, z, currStage]) - JuMP.value(ahusupplytemp[f, currStage])) / (p.zonedischargetemp_max - JuMP.value(ahusupplytemp[f, currStage]))
                    ...
                    df[Symbol("floor$(f)_zon$(z)_oveTSetDisAir_u")] = JuMP.value(zonedischargetemp[f, z, currStage])
                end
        end
...
function updatemodelparams(df::DataFrames.DataFrame, params::Dict)
    ...
    for fInd = 1:p.numfloors, zInd = 1:p.numzones, hInd = 1:o.numwindows
        JuMP.set_lower_bound(zoneflow[fInd, zInd, hInd], p.zoneflow_min[zInd])
        JuMP.set_upper_bound(zoneflow[fInd, zInd, hInd], p.zoneflow_max[zInd])
    end
end
